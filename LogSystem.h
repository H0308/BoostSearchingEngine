#ifndef bs_LogSystem_h
#define bs_LogSystem_h
...
namespace log_system
{
    ...
    // 默认的目录路径和文件路径
    const std::string g_d_dir_path = "./log/";
    const std::string g_d_file_path = "log.txt";
    ...
    {
    public:
        FileLogStrategy(const std::string &dir_path = g_d_dir_path, const std::string &file_path = g_d_file_path)
            : dir_path_(dir_path), file_path_(file_path)
        {
            // 先申请锁
            MutexGuard guard(lock_);
        ...
        virtual void printLog(const std::string &message) override
        {
            ...
            // 打开文件并追加写入
            const std::string path = dir_path_ + file_path_;
    ...
    public:
        ...
        LogHandler()
            : log_(std::make_shared<ConsoleLogStrategy>())
        {
        }
        ...
        void enableFileLog()
        {
            log_ = std::make_shared<FileLogStrategy>();
        }
        ...
        public:
            LogMessage(LogLevel level, const std::string &filename, int lineno, LogHandler &loghandler)
                : _time(getCurrentTime()), _level(level), _pid(getpid()), _filename(filename), _lineno(lineno), log_handler(loghandler)
            {
                std::stringstream ss;
                ss << "[" << _time << "] "
                   << "[" << level2string(_level) << "] "
            ...
            ~LogMessage()
            {
                ...
                if (log_handler.log_)
                    log_handler.log_->printLog(_message);
            }
        ...
        private:
            ...
            LogHandler &log_handler; // LogHandler对象
        };
